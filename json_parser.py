...
if os.path.exists("student_b92_implementation_status.json"):
    from quantum_network.interactive_host_b92 import InteractiveQuantumHostB92 as QuantumHost
    print("🔹 Using B92 Interactive Quantum Host")
else:
    from quantum_network.interactive_host import InteractiveQuantumHost as QuantumHost
    print("🔹 Using BB84 Interactive Quantum Host")
        ...
        for network_data in zone_data.get('networks', []):  # Use .get() for safety
            ...
            for host_data in network_data.get('hosts', []):
                if host_data['type'] == "ClassicalHost":
                    ...
                    # Use interactive quantum host with student implementation
                    # QuantumHost is already imported above based on B92/BB84 availability
                    host = QuantumHost(
                        address=host_data['address'],
                        location=tuple(host_data['location']),
                        network=network,
